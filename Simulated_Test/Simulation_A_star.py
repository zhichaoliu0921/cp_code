...
import numpy as np
from Waypoint_generation_CA.A_star import AStarPlanner
...
(-2.75, 8.75): {'ind': 15, 'radius': 0.58}, (3.25, 0.25): {'ind': 27, 'radius': 0.58}}
    ...
    starttime = time.time()
    a_star = AStarPlanner(min_x, min_y, max_x, max_y, resolution, obstalce_circle_dict, dis_potential_bound)
    ...
    starttime = time.time()
    traj = trajGenerator(waypoints, vel_st, vel_ed, max_vel=v_max, gamma=1e2)
    endtime = time.time()
