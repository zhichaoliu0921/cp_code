...
class trajGenerator:
    def __init__(self, waypoints, st_vel, ed_vel, st_acc, ed_acc, st_jerk, ed_jerk, st_snap, ed_snap, max_vel=5,
                 gamma=100):
        ...
        self.st_acc = st_acc
        self.ed_acc = ed_acc
        self.st_jerk = st_jerk
        self.ed_jerk = ed_jerk
        self.st_snap = st_snap
        self.ed_snap = ed_snap
    ...
    def get_constraints(self, T):
        ...
        #velocity start and end at rest
        ...
        # acceleration start and end at rest
        B[6 * n - 3, :] = self.st_acc
        B[6 * n + 1, :] = self.ed_acc

        # jerk start and end at rest
        B[6 * n - 2, :] = self.st_jerk
        B[6 * n + 2, :] = self.ed_jerk

        # jerk start and end at rest
        B[6 * n - 1, :] = self.st_snap
        B[6 * n + 3, :] = self.ed_snap
